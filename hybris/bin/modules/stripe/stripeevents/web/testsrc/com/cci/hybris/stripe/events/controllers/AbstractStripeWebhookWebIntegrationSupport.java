...
/**
 * Shared embedded-server integration-test helpers for Stripe webhook controller tests.
 */
public abstract class AbstractStripeWebhookWebIntegrationSupport extends ServicelayerTest {
    ...
    @Resource
    protected StripeCheckoutSessionService stripeCheckoutSessionService;
    ...
    protected void prepareLiveStripeWebhookConfiguration(final String siteUid) {
        ...
        Assume.assumeTrue("Stripe secret key must be configured for webhook HTTP integration tests.",
                isConfiguredValue(secretKey));
        Assume.assumeTrue("Stripe success URL must be configured for webhook HTTP integration tests.",
                isConfiguredValue(successUrl));
        Assume.assumeTrue("Stripe cancel URL must be configured for webhook HTTP integration tests.",
                isConfiguredValue(cancelUrl));
        Assume.assumeTrue("Stripe webhook secret must be configured for webhook HTTP integration tests.",
    ...
    protected OrderModel createOrder(final String orderCode, final String siteUid) {
        ...
        customer.setName("Stripe Webhook HTTP Tester");
    ...
    protected StripeCheckoutSessionData createCheckoutSession(final OrderModel order, final String siteUid) {
